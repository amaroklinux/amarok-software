...
locale.bindtextdomain('amarok-software', '/usr/share/locale')
locale.textdomain('amarok-software')
...
class MainWindow(object):
    def __init__(self, application):
        ...
        self.aboutdialog.set_program_name(_("Amarok Software Center"))
    ...
    def controlPSUpdate(self):
        if self.Server.connection and self.UserSettings.usercodename == "rolling" and not self.isbroken:
            user_version = self.Package.installedVersion("amarok-software")
            ...
            if user_version is not None and server_version != "":
                ...
                if version < 0:
                    self.notify(message_summary=_("Amarok Software Center | New version available"),
    ...
    def server(self):
        ...
        conffile = "/etc/amarok/amarok-software.conf"
        self.repo_perm = 1
        self.myapps_perm = 1
        try:
    ...
    def getSystemAppIcon(self, app, size=64, notify=False, myappicon=False):
        try:
            ...
            try:
                ...
                if notify:
                    try:
                        appicon = Gtk.IconTheme.get_default().load_icon("amarok-software", size,
                                                                        Gtk.IconLookupFlags(16))
                    except:
                        try:
                ...
                if notify:
                    try:
                        appicon = GdkPixbuf.Pixbuf.new_from_file_at_size(
                            self.Server.cachedir + "appicons/amarok-software.svg", size, size)
                    except:
                        try:
                            appicon = Gtk.IconTheme.get_default().load_icon("gtk-missing-image", size,
                                                                            Gtk.IconLookupFlags(16))
    ...
    def set_stack_n_search(self, id):
        # id:  1 = pardus, 2 = repo, 3 = queue, 4 = myapps
        if id == 1:
    ...
    def on_pref_tip_clicked(self, button):
        ...
        if button.get_name() == "tip_usi":
            ...
            self.prefTipLabel.set_text("{}\n{}".format(
                _("Hide applications offered from external repositories (other than Official Amarok repository)."),
    ...
    def notify(self, message_summary="", message_body=""):
        try:
            ...
            if message_summary == "" and message_body == "":
                ...
                notification = Notify.Notification.new(message_summary, message_body, "amarok-software")
            notification.show()
        except Exception as e:
            print("{}".format(e))
    ...
    def onSysProcessExit(self, pid, status):
        if self.externalactioned:
            ...
            if status == 0 and not self.error:
                self.notify(message_summary=_("Amarok Software Center"), message_body=_("Repo Activation Completed"))
